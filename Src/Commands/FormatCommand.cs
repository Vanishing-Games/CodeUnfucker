...
namespace CodeUnfucker.Commands
{
    ...
    /// 内置格式化命令
    /// </summary>
    public class FormatCommand : BaseCommand
    {
        public override string Name => "format";
        public override string Description => "使用内置格式化器格式化代码";

        public FormatCommand(ILogger logger, IFileService fileService) 
        ...
        {
            Logger.LogInfo($"开始使用内置格式化器格式化代码，扫描路径: {path}");
            ...
            try
            {
                ...
                else if (FileService.DirectoryExists(path))
                {
                    ...
                    Logger.LogInfo($"格式化完成！成功: {successCount}, 失败: {failureCount}");
        ...
        protected override async Task<bool> ProcessSingleFileAsync(string filePath)
        {
            try
            {
                ...
                if (writeSuccess)
                {
                    Logger.LogInfo($"✅ 格式化完成: {filePath}");
                    return true;
                }
                else
                {
                    Logger.LogError($"写入格式化后的代码失败: {filePath}");
                    return false;
                }
            }
            catch (Exception ex)
            {
                Logger.LogError($"格式化文件失败 {filePath}", ex);
                return false;
            }
        }
    }
